...
1. **Notifica lock screen** = campo FRONT `announcement` (auxiliary) con `value` = **solo token invisibile** (cambia a ogni push, nessuna 4ª colonna visibile) e `changeMessage` = **testo `screen_alert` + `%@`** (iOS esige `%@` e lo sostituisce col valore invisibile → mostra solo il testo). iOS **ignora** `changeMessage` senza `%@`; i back field aggiornano **in silenzio** e **non devono mai avere `changeMessage` vuoto** (scatena la generica); l'header è inaffidabile. NON tornare a: testo visibile nel value, aux `screen_alert` col puntino, back `wallet_push_alert`, header hint con changeMessage.
